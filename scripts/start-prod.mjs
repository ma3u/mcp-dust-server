...
/**
 * Production-ready multi-instance server launcher for MCP-Dust server
 * Works with compiled JavaScript files
 */
...
const instanceCount = parseInt(args[0], 10) || 1;
...
function createInstanceEnv(index) {
  return {
    ...process.env,
    START_MODE: 'server',
    INSTANCE_ID: `prod-instance-${index}`,
    MCP_MIN_PORT: '5001',
    MCP_MAX_PORT: '5050',
...
console.log(`Starting ${instanceCount} MCP server instance${instanceCount > 1 ? 's' : ''} in production mode...`);
...
function startInstance(index) {
  return new Promise((resolve) => {
    ...
    // Start the server process with compiled JavaScript
...
async function startAllInstances() {
  try {
    ...
    if (!fs.existsSync(distDir) || !fs.existsSync(resolve(distDir, 'index.js'))) {
