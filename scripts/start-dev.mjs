 ...
 * Uses ts-node directly to avoid module resolution issues
 */
...
function createInstanceEnv(index) {
  return {
    ...process.env,
    START_MODE: 'server',
    INSTANCE_ID: `dev-instance-${index}`,
    MCP_MIN_PORT: '5001',
    MCP_MAX_PORT: '5050',
...
/**
 * Start a single MCP server instance using ts-node directly
 * 
...
function startInstance(index) {
  return new Promise((resolve) => {
    ...
    // Use ts-node directly to avoid module resolution issues
    const serverProcess = spawn('npx', ['ts-node', '--transpileOnly', '--project', 'tsconfig.json', 'src/index.ts'], {
      cwd: projectRoot,
      env,
